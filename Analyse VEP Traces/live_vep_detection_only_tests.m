function [output_msg, signal_noise_peak_to_peak, signal_peak_to_peak]= live_vep_detection_only_tests(working_directory, signal_power,pulse_width)
    ...
    number_of_tests = length(test_list);
    ...
    for i = 1:number_of_tests
        ...
        % generate full path
        test_file = fullfile(latest_folder,strcat(test_name, num2str(test_list(i)), '.csv'));
    ...
    y_min = nan;
    y_max = nan;
    pk_to_pk_time_window = 100;
    inner_max_window = 120;
    noise_detect_time = 320;
    noise_window = 120;
    user_title = "";
    save_figures = false;
    
    % calculate peak-to-peak normalized to noise level
    [peak_to_peak_amplitudes, signal_noise_peak_to_peak_amplitude, ~] = calculate_peak_to_peak_only_tests(raw_data, laser_intensity, normalize_to_noise, y_min, y_max, pk_to_pk_time_window, inner_max_window, noise_detect_time, noise_window, user_title, save_figures, pulse_width);
