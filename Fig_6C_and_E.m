...
Field_distal = fscanf(fopen('MP20 new Field/slice_1.txt', 'r'), '%f', [501 501]);
Field_distal = Field_distal';
Field_proximal = fscanf(fopen('MP20 new Field/slice_70.txt', 'r'), '%f', [501 501]);
...
Field_distal_centre = Field_distal(250,:);
Field_proximal_centre = Field_proximal(250,:);


fig = figure;
Field_amp = (Field_distal_centre - Field_proximal_centre)*(0.15/0.2);
x = -750:3:750;
plot(x, Field_amp, "k");
xlim([-750 750]);
%ylim([-4 12]);
...
% hold on;
% Field_amp = (Field_distal_centre - Field_proximal_centre)*(3);
% plot(x, Field_amp, "g");

Avg_Field_amp = movmean(Field_amp, 33);
hold on
plot(x, Avg_Field_amp, 'Color', "r");
legend("Voltage across BC", "100 µm Spatial average ");
